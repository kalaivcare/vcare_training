...
class QuickUpdateController extends Controller
{
    ...
    public function courseabc($id)
    {
     ...
     public function workQuick($id)
    {
    	$course = Workshop::findorfail($id);

    	if($course->status ==0)
    	{
    		DB::table('workshops')->where('id','=',$id)->update(['status' => "1"]);	
    		return back()->with('success','Status changed to Active !');
    	}
    	else
    	{
    		DB::table('workshops')->where('id','=',$id)->update(['status' => "0"]);
    		return back()->with('delete','Status changed to Deactive !');
    	}
    }

    public function workabc($id)
    {
        $course = Workshop::findorfail($id);

        if($course->featured ==0)
        {
            DB::table('workshops')->where('id','=',$id)->update(['featured' => "1"]);   
            return back()->with('success','Featured changed to YES !');
        }
        else
        {
            DB::table('workshops')->where('id','=',$id)->update(['featured' => "0"]);
            return back()->with('delete','Featured changed to NO !');
        }
    }
