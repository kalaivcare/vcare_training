...
class ApplyCouponController extends Controller
{
    public function applycoupon(Request $request)
    {
        ...
        $course = Order::where('user_id',$refer->id)->first();
        if($cart->disamount!=null){
            return back()
            ->with('fail', 'Already discount is applied to your cart !');
        }
       }
    	...
    	if(isset($coupon)){
    		...
    		if($coupon->expirydate >= $date)
    		{
    			if($coupon->maxusage != 0)
    			{
                   ...
                   elseif($coupon->link_by == 'workshop')
                    {

                        return $this->validCouponForWorkshop($coupon);

                    }
                     elseif($coupon->link_by == 'user')
                    {

                        return $this->validCouponForuser($coupon);
    ...
    public function validCouponForCourse($coupon)
    {
        ...
        if (isset($cart))
        {
             if ($cart->price == null) {
                return back()
                ->with('fail', 'This coupon is not applicable !');
            } else {
    ...
    public function validCouponForWorkshop($coupon)
    {
    	$cart = Cart::where('workshop_id', '=', $coupon['workshop_id'])->where('user_id', '=', Auth::user()->id)->first();
        ...
        if (isset($cart)) {
            if ($cart->price == null) {
                ...
                if ($cart->workshop_id == $coupon->workshop_id) {
                    if ($coupon->distype == 'per') {
                        $per = $cart->offer_price * $coupon->amount / 100;
                    } else {
                        $per = $coupon->amount;
                    ...
                    Cart::where('workshop_id', '=', $coupon['workshop_id'])->where('user_id', '=', Auth::user()
                    ->id)
                    ->update(['distype' => 'workshop', 'disamount' => $per]);
                    Cart::where('workshop_id', '!=', $coupon['workshop_id'])->where('user_id', '=', Auth::user()
                    ->id)
                    ->update(['distype' => null, 'disamount' => null]);
      ...
      public function validCouponForuser($coupon)
    {
    	$cart = Cart::where('user_id', '=', $coupon['user_id'])->where('user_id', '=', Auth::user()->id)->first();
        ...
        if (isset($cart)) {
            if ($cart->price == null) {
                ...
                if ($cart->user_id == $coupon->user_id) {
                    ...
                    // Putting a session//
                    Session::put('coupanapplied', ['code' => $coupon->code, 'cpnid' => $coupon->id, 'discount' => $per, 'msg' => "$coupon->code is applied !", 'appliedOn' => 'user']);

                    Cart::where('user_id', '=', $coupon['user_id'])->where('user_id', '=', Auth::user()
                    ->id)
                    ->update(['distype' => 'user', 'disamount' => $per]);
                    Cart::where('user_id', '!=', $coupon['user_id'])->where('user_id', '=', Auth::user()
                    ->id)
                    ->update(['distype' => null, 'disamount' => null]);
                    ...
                    return back()
                    ->with('fail', 'Sorry, This coupon is not applicable to you ! ');
                }
            }
        }
        else
        {
            return back()->with('fail', 'Sorry, This coupon is not applicable to you ! ');
        }
    }
    ...
    public function validCouponForCategory($coupon)
    {
        ...
        if (count($catcart) > 0)
        {
            ...
            if ($coupon->minamount != 0)
            {
                ...
                if ($total >= $coupon->minamount && $c->price !=Null)
                {
            ...
            else
            {
                 if ($c->price != null) {
                    ...
                    return back()->with('fail','This coupon is not applicable to you !');
                }
            }
