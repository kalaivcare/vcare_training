        ...
        <form action="{{ route('coupon.update',$coupan->id) }}" method="POST">
          ...
          <div class="box-body">
              ...
              <div class="form-group">
                <label>{{ __('adminstaticword.Linkedto') }}: <span class="redstar">*</span></label>
                  ...
                  <select required="" name="link_by" id="link_by" class="form-control js-example-basic-single">
                    ...
                    <option {{ $coupan->link_by == 'workshop' ? "selected" : ""}} value="workshop">Link to Workshop</option>
                    <option {{ $coupan->link_by == 'user' ? "selected" : ""}} value="user">Link to User</option>
                  </select>
              ...
              <div style="{{ $coupan->link_by == 'course' ? "" : "display: none"}}" id="probox" class="form-group">
                ...
                <div style="{{ $coupan->link_by == 'workshop' ? "" : "display: none"}}" id="workbox" class="form-group">
                <label>Select Workshop : <span class="redstar">*</span> </label>
                <br>
                <select id="work_id" name="workshop_id" class="js-example-basic-single form-control">
                    <option value="none" selected disabled hidden> 
                       {{ __('adminstaticword.SelectanOption') }}
                    </option>
                    @foreach(App\Workshop::where('status','1')->get() as $product)
                      @if($product->type == 1)
                        <option {{ $coupan->workshop_id == $product->id ? 'selected' : "" }} value="{{ $product->id }}">{{ $product['title'] }}</option>
                      @endif
                    @endforeach
                </select>
              </div>
              
               <div style="{{ $coupan->link_by == 'user' ? "" : "display: none"}}" id="userbox" class="form-group">
                <label>Select User : <span class="redstar">*</span> </label>
                <br>
                <select id="user_id" name="user_id" class="js-example-basic-single form-control">
                    <option value="none" selected disabled hidden> 
                       {{ __('adminstaticword.SelectanOption') }}
                    </option>
                    @foreach(App\User::where('role','user')->get() as $product)
                     
                        <option {{ $coupan->user_id == $product->id ? 'selected' : "" }} value="{{ $product->id }}">{{ $product['fname'] }}{{ $product['lname'] }}</option>
