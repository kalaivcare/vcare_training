    ...
    <div class="col-md-12">
      ...
      <div class="table-responsive">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
            <?php $i=0;?>
            @foreach($workwhatlearn as $cat)
              <tr>
                <?php $i++;?>
                  <td><?php echo $i;?></td>
                <td>{{$cat->workshop->title}}</td>
                ...
                <td>
                  <a class="btn btn-success btn-sm" href="{{url('workwhatlearn/'.$cat->id)}}"><i class="glyphicon glyphicon-pencil"></i></a>
                </td> 
                <td>
                  <form  method="post" action="{{url('workwhatlearn/'.$cat->id)}}" data-parsley-validate class="form-horizontal form-label-left">
                    {{ csrf_field() }}
                    {{ method_field('DELETE') }}
        ...
        <div class="box box-primary">
          <div class="panel panel-sum">
            <div class="modal-body">
              <form id="demo-form2" method="post" action="{{ route('workwhatlearn.store') }}" data-parsley-validate class="form-horizontal form-label-left">
                {{ csrf_field() }}
                <select name="course_id" class="form-control display-none">
                  <option class="display-none" name="workshop_id"  value="{{ $cor->id }}">{{ $cor->title }}</option>
                </select>
                <input name="workshop_id" value="{{ $cor->id }}" type="hidden">
                <div class="row">
                  <div class="col-md-6">
