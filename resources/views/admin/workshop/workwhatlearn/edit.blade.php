      ...
      <div class="box box-primary">
        ...
        <div class="box-body">
          <div class="form-group">
            <form id="demo-form" method="post" action="{{url('workwhatlearn/'.$cate->id)}}" data-parsley-validate class="form-horizontal form-label-left">
              {{ csrf_field() }}
              {{ method_field('PUT') }}
             ...
             <label class="display-none" for="exampleInputSlug">{{ __('adminstaticword.SelectCourse') }}</label>
              <select  name="workshop_id" class="form-control col-md-7 col-xs-12 display-none">
                ...
                @foreach($courses as $cou)
                  <option class="display-none" value="{{ $cou->id }}"{{$cate->workshop->id == $cou->id  ? 'selected' : ''}}>{{ $cou->title}}</option>
                @endforeach
