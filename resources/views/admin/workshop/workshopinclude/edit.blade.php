...
@section('title', 'Edit Workshop Include - Admin')
@section('body')
    ...
    <div class="col-xs-12">
      ...
      <div class="box box-primary">
        ...
        <div class="box-header with-border">
          <h3 class="box-title"> {{ __('adminstaticword.Edit') }} Workshop Include</h3>
        </div>
        <!-- /.box-header -->
        <!-- form start -->
        <div class="box-body">
          <div class="form-group">
            <form id="demo-form" method="post" action="{{url('workshopinclude/'.$cate->id)}} "data-parsley-validate class="form-horizontal form-label-left">
              {{ csrf_field() }}
              {{ method_field('PUT') }}

                <label class="display-none" for="exampleInputSlug">{{ __('adminstaticword.SelectCourse') }}</label>
                <select name="workshop_id" class="form-control col-md-7 col-xs-12 display-none">
        
                  @foreach($courses as $cou)
                    <option  value="{{ $cou->id }}" {{$cate->workshops->id == $cou->id  ? 'selected' : ''}}>{{ $cou->title}}</option>
                  @endforeach
                </select>
