  ...
  <div class="col-md-12">
    ...
    <div class="table-responsive">
      <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
          <?php $i=0;?>
          @foreach($workshopinclude as $cat)
            <tr>
              <?php $i++;?>
              <td><?php echo $i;?></td>
              <td>{{$cat->workshops->title}}</td>
              <td>{{$cat->icon}}</td>
              ...
              <td>
                  <a class="btn btn-success btn-sm" href="{{url('workshopinclude/'.$cat->id)}}">
                    <i class="glyphicon glyphicon-pencil"></i></a>
              </td>              

              <td>
                <form  method="post" action="{{url('workshopinclude/'.$cat->id)}}
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">{{ __('adminstaticword.Add') }} Workshop Include</h4>
      </div>
       <div class="box box-primary">
          <div class="panel panel-sum">
            <div class="modal-body">
              <form id="demo-form2" method="post" action="{{ route('workshopinclude.store') }}" data-parsley-validate class="form-horizontal form-label-left">
                {{ csrf_field() }}
     
                <select class="display-none" name="workshop_id" class="form-control">
