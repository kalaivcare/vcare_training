  ...
  <div class="col-xs-12">
    <div class="box box-primary">
      <div class="box-header with-border">
        ...
        <a class="btn btn-info btn-sm" href="{{url('workshop/create')}}">
          <i class="glyphicon glyphicon">+</i> {{ __('adminstaticword.Add') }} Workshops
        </a>
      </div>
              ...
              <tbody>
                <?php $i=0;?>
                  @if(Auth::User()->role == "admin")
                    @foreach($workshop as $cat)
                      <?php $i++;?>
                      <tr>
                        <td><?php echo $i;?></td>
                        <td>
                          @if($cat['preview_image'] !== NULL && $cat['preview_image'] !== '')
                              <img src="images/workshop/<?php echo $cat['preview_image'];  ?>" class="img-responsive" >
                          @else
                        ...
                        <td>{{$cat->slug}}</td>
                        <td>
                          <form action="{{ route('workshop.featured',$cat->id) }}" method="POST">
                            {{ csrf_field() }}
                        ...
                        <td>
                          <form action="{{ route('workshop.quick',$cat->id) }}" method="POST">
                            {{ csrf_field() }}
                        ...
                        <td>
                          <a class="btn btn-success btn-sm" href="{{ route('workshop.show',$cat->id) }}">
                          <i class="glyphicon glyphicon-pencil"></i></a>
                        </td>

                        <td>
                          <form  method="post" action="{{url('workshop/'.$cat->id)}}
                  ...
                  @else
                    ...
                    @foreach($cors as $cor)
                      ...
                      <tr>
                        ...
                        <td>
                          <a class="btn btn-primary btn-sm" href="{{ route('workshop.show',$cor->id) }}">
                          <i class="glyphicon glyphicon-pencil"></i></a>
                        </td>
